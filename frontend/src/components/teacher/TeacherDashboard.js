  ...
  return (
    <div className="space-y-8">
      ...
      <section aria-labelledby="descriptors-title">
        <div className="mb-4"><p className="text-xs font-black uppercase tracking-[0.18em] text-fuchsia-600">Leitura curricular</p><h3 id="descriptors-title" className="mt-1 flex items-center gap-2 text-2xl font-black"><Target className="text-fuchsia-500" /> Evidências por descritor</h3></div>
        {data.descriptors?.length ? <div className="grid gap-4 md:grid-cols-2">{data.descriptors.map(descriptor => <Card key={descriptor.id} className="border-0 shadow-[0_12px_35px_rgba(15,23,42,.07)]"><div className="flex items-start justify-between gap-3"><div><Badge tone={descriptor.evidencePct >= 70 ? 'success' : 'warning'}>{descriptor.code}</Badge><h4 className="mt-3 font-black text-slate-900">{descriptor.description}</h4></div><strong className="text-2xl text-fuchsia-700">{descriptor.evidencePct}%</strong></div><Progress value={descriptor.evidencePct} label="Acertos observados" className="mt-5" /><p className="mt-3 text-xs text-slate-500">{descriptor.correctAnswers} acertos em {descriptor.totalAnswers} respostas · {descriptor.studentsWithEvidence} estudante{descriptor.studentsWithEvidence === 1 ? '' : 's'} com evidência</p></Card>)}</div> : <EmptyState title="Nenhum descritor medido" description="Os indicadores curriculares surgem após quizzes vinculados a descritores." />}
      </section>
