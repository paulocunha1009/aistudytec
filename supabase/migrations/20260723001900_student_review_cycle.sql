...
alter table public.review_queue
  drop constraint if exists review_queue_student_id_topic_id_skill_status_key;

with ranked_pending as (
  select id,
    row_number() over (
      partition by student_id, skill
      order by due_at desc, created_at desc, id
    ) as position
...
create unique index if not exists review_queue_one_pending_skill_idx
  on public.review_queue(student_id, skill)
  where status = 'pending';
...
begin
  ...
  for answer_item in select * from jsonb_array_elements(p_answers) loop
    ...
    if selected_option = '' or correct_option is null then
      raise exception using errcode = '22023', message = 'Resposta ou gabarito inválido';
    end if;
  ...
  select coalesce(jsonb_agg(distinct queue.skill), '[]'::jsonb)
  into completed_reviews
  from public.review_queue queue
  where queue.student_id = actor_id
    and queue.status = 'pending'
    and queue.skill in (
      select distinct attempt_answer.skill
      from public.quiz_attempt_answers attempt_answer
      where attempt_answer.attempt_id = created_attempt_id
    );
  ...
  for skill_row in
    select mastery.skill, mastery.mastery_pct
    from public.skill_mastery mastery
    where mastery.student_id = actor_id
      and mastery.skill in (
        select distinct attempt_answer.skill
        from public.quiz_attempt_answers attempt_answer
        where attempt_answer.attempt_id = created_attempt_id
      )
  loop
    if skill_row.mastery_pct < 70 then
      insert into public.review_queue (student_id, topic_id, skill, due_at, status)
      values (actor_id, p_topic_id, skill_row.skill, now() + interval '3 days', 'pending')
      on conflict (student_id, skill) where status = 'pending'
      do update set topic_id = excluded.topic_id, due_at = excluded.due_at;
    end if;
  end loop;
  ...
  insert into public.audit_events (
    ...
    jsonb_build_object(
      'topicId', p_topic_id, 'score', score_value, 'total', total_value,
...
comment on index public.review_queue_one_pending_skill_idx is
  'Mantém somente uma revisão pendente por aluno e habilidade, preservando o histórico resolvido.';
